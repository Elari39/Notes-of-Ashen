...
Notes of Ashen 是一个前后端分离的个人博客项目。后端使用 Go 和 go-zero 风格组织代码，提供用户认证、文章管理、分类标签、后台管理、操作日志等能力；前端使用 React、TypeScript、Vite 和 Tailwind CSS 构建博客展示与管理界面。

## 功能特性

- 用户认证：注册、登录、退出、刷新 Token。
- 双 Token 机制：Access Token 使用 JWT，Refresh Token 哈希后持久化，并结合 Redis 校验。
- 文章管理：创建、编辑、删除、发布、归档、公开列表、公开详情。
- 分类与标签：公开读取，管理员创建、更新、删除。
- 后台能力：用户列表、用户状态管理、操作日志查看。
- 异步日志：通过 RabbitMQ 投递注册、登录、文章操作等事件，并写入 `operation_logs`。
- 统一响应：接口成功返回 `{ "code": 0, "message": "success", "data": ... }`。

## 技术栈

- 后端：Go 1.25、go-zero REST、MySQL、Redis、RabbitMQ、JWT、bcrypt。
- 前端：React 18、TypeScript、Vite、Tailwind CSS、Zustand、Axios。
- 文档与脚本：API 文档位于 [docs/API.md](docs/API.md)，数据库初始化脚本位于 [deploy/mysql/schema.sql](deploy/mysql/schema.sql)。
...
这些默认账号和密码只适合本地开发，生产环境必须替换。

## 后端配置

后端配置文件位于 [etc/notes-of-ashen.yaml](etc/notes-of-ashen.yaml)：
...
前端位于 `frontend/`，开发服务器默认端口为 `3000`，并将 `/api` 代理到 `http://127.0.0.1:19000`。
...
3. 注册或登录成功后返回 `accessToken` 和 `refreshToken`。
4. 访问受保护接口时添加请求头：
...
5. Access Token 过期后，调用 `POST /api/v1/auth/refresh` 换取新的 Token。
