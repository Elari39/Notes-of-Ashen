...
const fieldNames: Record<string, string> = {
  ...
  model: '模型',
  apiKey: 'API Key',
  firstByteTimeoutSeconds: '首字等待时间',
  streamTimeoutSeconds: '流式输出超时',
  nonStreamTimeoutSeconds: '非流式输出超时',
  status: '状态',
  q: '搜索关键词',
};
...
const exactMessages: Record<string, string> = {
  ...
  'tag not found': '标签不存在',
  'ai assistant is disabled': 'AI 辅助未启用，请先在后台 AI 配置中启用。',
  'ai assistant is not configured': 'AI 辅助尚未配置，请检查 Base URL、API Key 和模型。',
  'ai response is invalid': 'AI 返回内容格式不正确，请重试。',
  'baseUrl format is invalid': 'Base URL 必须以 http:// 或 https:// 开头，或留空',
  'firstByteTimeoutSeconds is invalid': '首字等待时间必须在 1 到 1800 秒之间',
  'streamTimeoutSeconds is invalid': '流式输出超时必须在 1 到 1800 秒之间',
  'nonStreamTimeoutSeconds is invalid': '非流式输出超时必须在 1 到 1800 秒之间',
  'streamTimeoutSeconds must be greater than or equal to firstByteTimeoutSeconds': '流式输出超时不能小于首字等待时间',
  'nonStreamTimeoutSeconds must be greater than or equal to firstByteTimeoutSeconds': '非流式输出超时不能小于首字等待时间',
