...
本文件承载 `Notes of Ashen` 的完整部署细节：环境变量清单、各可选能力专项配置、Docker 与 1Panel 部署流程、发布回滚、数据持久化与常见问题。概览与快速开始请回到 [README](../README.md)。

> ⚠️ 所有真实密码、密钥、Token 只写入 `.env` 或部署平台环境变量，不要写入 README、Issue、提交记录或截图。
...
至少需要检查并替换这些值。模板中的 `<REPLACE_...>` 只用于提示，不是可用默认值；后端启动时会拒绝空值、示例占位值和过短的 JWT 密钥。

- `APP_HOST`：后端监听地址，默认 `0.0.0.0`；仅本地非 Docker 启动使用。
- `APP_PORT`：后端监听端口，默认 `19000`；仅本地非 Docker 启动使用，Compose 内部固定为 `19000`。
- `APP_TIMEOUT`：请求超时（毫秒），默认 `610000`。
...
- `APP_AUTH_ACCESS_EXPIRE`：accessToken 有效期（秒），默认 `1800`。
- `APP_AUTH_REFRESH_EXPIRE`：refreshToken 有效期（秒），默认 `604800`。
...
- `COMPOSE_PROFILES`：可选 Compose 服务 profile 组合，默认为空（仅 Web、API、MySQL、Redis）；可组合 `messaging`、`search`、`rag`，如 `messaging,search,rag`。启用 profile 时必须同步打开对应能力开关，否则 `config-check` 会失败。
...
- `MEILI_ENV`：Meilisearch 运行模式，默认 `production`；仅临时开发时显式设为 `development`。
- `APP_RAG_ENABLED`：是否启用 RAG 知识库问答，默认 `false`；启用时必须同时设置 `COMPOSE_PROFILES=rag`。
- `APP_QDRANT_URL`：Qdrant 服务地址，Compose 默认 `http://qdrant:6333`。
- `APP_QDRANT_API_KEY`：Qdrant API Key；内置 Qdrant 不暴露宿主机端口、默认留空，接入受保护的外部实例时填写。
- `APP_QDRANT_COLLECTION`：Qdrant 集合名，默认 `rag_articles`。
...
已有数据卷首次升级会建立迁移记录并按历史顺序执行全部迁移。迁移序列持续演进，近期版本包括 refresh token 清理索引（`000025`）、RAG 基础表（`000026`）、RAG 重建租约（`000027`）与 Embedding 维度默认值（`000028`）。历史链包含清理无效头像、清理孤儿文章版本和删除旧流量表的前向破坏性操作。升级前必须完成可恢复的数据库备份；迁移失败时不要修改已发布的 SQL 文件，应在修复后重新执行或新增前向修复迁移。回滚依赖备份恢复。
...
内置 Redis 模式下，确认 `.env` 中的 `APP_REDIS_ADDR=redis:6379`、`APP_REDIS_PASSWORD` 与 Redis 容器密码一致，且 `redis` 容器健康；空密码表示无认证。外部 Redis 模式下，使用 [docker-compose.external-redis.yml](../docker-compose.external-redis.yml) 覆盖文件启动，并确认这两个变量与外部实例一致。Redis 不可用或密码错误会使 API 按 fail-fast 策略启动失败。
